...
var ViewModel = function (list) {
	...
	if(window.sessionStorage.feedstate){
		var newStateObj = JSON.parse(window.sessionStorage.feedstate);
		console.log(newStateObj);
	}
	this.defaultCurrentPage = newStateObj ? Math.ceil(newStateObj.currentPage) : 1;	
	this.defaultSortBy = newStateObj ? newStateObj.sortBy : 'Title';
	this.defaultSearchQuery = newStateObj ? newStateObj.searchQuery : '';
	this.searchQuery = ko.observable(this.defaultSearchQuery);
	this.pageSize = 10;
	this.currentPage = ko.observable(this.defaultCurrentPage);
	this.cardList = list;
	this.sortByList = ['Date', 'Title'];
	this.sortBy = ko.observable(this.defaultSortBy);
	this.length = list.length;
	this.oldSearchQuery = this.searchQuery().toLowerCase();
	this.numberOfPages = ko.observable(list.length % this.pageSize === 0 ? list.length / this.pageSize : Math.ceil(list.length / this.pageSize + 1));
	this.numberOfPagesArray = ko.computed(function () {
		var list = [];
		var length = parseInt(currentScope.numberOfPages(), 10);
	...
	this.currentList = ko.pureComputed({
		read: function () {
			...
			if (this.oldSearchQuery !== search) {
				this.currentPage(1);
			}
			this.oldSearchQuery = search;
			var filteredList = list.filter(function (item) {
				return item.name.toLowerCase().indexOf(search) >= 0 || item.description.toLowerCase().indexOf(search) >= 0;
			}).sort(function (item1, item2) {
				if (currentScope.sortBy() === 'Title') {
				...
				if (currentScope.sortBy() === 'Date') {
					a = new Date(item1.dateLastEdited);
					b = new Date(item2.dateLastEdited);
					return a > b ? -1 : a < b ? 1 : 0;
				}
			});
			this.numberOfPages(filteredList.length % this.pageSize === 0 ? filteredList.length / this.pageSize : Math.ceil(filteredList.length / this.pageSize));
			this.updateState(this.currentPage(),this.sortBy(),this.searchQuery());
			return filteredList.slice((this.currentPage() - 1) * this.pageSize, (this.currentPage() - 1) * this.pageSize + this.pageSize);
		},
		owner: this
	});
	...
	this.updateState = function(currentPage, sortBy, searchQuery){
		var stateObj = {
			'currentPage': currentPage,
			'sortBy': sortBy,
			'searchQuery': searchQuery
		};
		window.sessionStorage.feedstate = JSON.stringify(stateObj);
	}
};

$(document).ready(function () {

	ko.applyBindings(new ViewModel(cardList));
});